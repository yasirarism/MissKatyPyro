...
async def stackoverflow(client, message):
    ...
    r = (requests.get(f"https://api.stackexchange.com/2.3/search/excerpts?order=asc&sort=relevance&q={message.command[1]}&accepted=True&migrated=False¬ice=False&wiki=False&site=stackoverflow")).json()
    hasil = ""
...
async def gsearch(client, message):
    ...
    try:
        ...
        html = requests.get(f"https://www.google.com/search?q={query}&gl=id&hl=id&num=17", headers=headers)
...
async def imdb1_search(client, message):
    ...
    if len(message.command) > 1:
        ...
        buttons.add(*[(InlineKeyboardButton(text=count, callback_data=f"imdbid#{movieID}")) for count, movie in enumerate(IMDBDATA, start=1)])
