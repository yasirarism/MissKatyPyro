...
async def index_files(_, query):
    ...
    if lock.locked():
        return await query.answer('Wait until previous process complete.',
                                  show_alert=True)
    msg = query.message

    await query.answer('Processing...⏳', show_alert=True)
    if int(from_user) != 617426792:
        await app.send_message(
            int(from_user),
    ...
    try:
        ...
        chat = chat
    await index_files_to_db(int(lst_msg_id), chat, msg, app)


@app.on_message((filters.forwarded | (filters.regex(
    "(https://)?(t\.me/|telegram\.me/|telegram\.dog/)(c/)?(\d+|[a-zA-Z_0-9]+)/(\d+)$"
)) & filters.text) & filters.private & filters.incoming)
async def send_for_index(app, message):
    if message.text:
        regex = re.compile(
            "(https://)?(t\.me/|telegram\.me/|telegram\.dog/)(c/)?(\d+|[a-zA-Z_0-9]+)/(\d+)$"
        )
    ...
    try:
        k = await app.get_messages(chat_id, last_msg_id)
    except:
        return await message.reply(
    ...
    if message.from_user.id == 617426792:
        buttons = [[
            InlineKeyboardButton(
                'Yes',
                callback_data=
                f'index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}')
        ], [
    ...
    if type(chat_id) is int:
        try:
            link = (await app.create_chat_invite_link(chat_id)).invite_link
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(
                'Reject Index',
                callback_data=
                f'index#reject#{chat_id}#{message.message_id}#{message.from_user.id}'
            ),
        ]
    ]
    reply_markup = InlineKeyboardMarkup(buttons)
...
@app.on_message(filters.command('setskip') & filters.user(617426792))
...
async def index_files_to_db(lst_msg_id, chat, msg, bot):
    ...
    async with lock:
        try:
            ...
            temp.CANCEL = False
            media = await bot.get_messages(chat, lst_msg_id, temp.CURRENT)
