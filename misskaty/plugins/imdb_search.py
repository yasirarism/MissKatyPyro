...
async def imdb_choose(self: Client, ctx: Message):
    ...
    buttons.row(
        InlineButton("🇺🇸 English",
                     f"imdbcari#eng#{ranval}#{ctx.from_user.id}"),
        InlineButton("🇮🇩 Indonesia",
                     f"imdbcari#ind#{ranval}#{ctx.from_user.id}"),
    )
    buttons.row(
        InlineButton("🚩 Set Default Language", f"imdbset#{ctx.from_user.id}"))
...
async def imdblangset(self: Client, query: CallbackQuery):
    ...
    msg = await query.message.edit_caption(
        "<i>Please select available language below..</i>",
        reply_markup=buttons)
    try:
...
async def imdbsetlang(self: Client, query: CallbackQuery):
    ...
    if langset == lang:
        return await query.answer(f"⚠️ Your Setting Already in ({langset})!",
                                  True)
    if lang == "eng":
...
async def imdb_search_id(kueri, message):
    ...
    try:
        ...
        for num, movie in enumerate(res, start=1):
            ...
            BTN.append(
                InlineKeyboardButton(
                    text=num,
                    callback_data=
                    f"imdbres_id#{message.from_user.id}#{movieID}",
                ))
        BTN.extend((
            InlineKeyboardButton(
                text="🚩 Language",
        ...
        try:
            await msg.wait_for_click(from_user_id=message.from_user.id,
                                     timeout=30)
        except ListenerTimeout:
            try:
                await msg.edit_caption(
                    "😶‍🌫️ Waktu Habis. Task Telah Dibatalkan!")
            except MessageIdInvalid:
                pass
        ...
        for num, movie in enumerate(res, start=1):
            ...
            BTN.append(
                InlineKeyboardButton(
                    text=num,
                    callback_data=
                    f"imdbres_en#{message.from_user.id}#{movieID}",
                ))
        BTN.extend((
            InlineKeyboardButton(
                text="🚩 Language",
    ...
    if lang == "ind":
        ...
        try:
            ...
            return await query.message.edit_caption(
                "⚠️ Callback Query Sudah Expired!")
        await query.message.edit_caption(
        ...
        try:
            ...
            for num, movie in enumerate(res, start=1):
                ...
                if year := movie.get("yr"):
                    ...
                    year = "(N/A)"
                typee = movie.get("q", "N/A").replace("feature",
                                                      "movie").title()
                movieID = re.findall(r"tt(\d+)", movie.get("id"))[0]
                msg += f"{num}. {title} {year} - {typee}\n"
                BTN.append(
        ...
        try:
            ...
            return await query.message.edit_caption(
                "⚠️ Callback Query Expired!")
        await query.message.edit_caption(
            "<i>🔎 Looking in the IMDB Database..</i>")
        msg = ""
        buttons = InlineKeyboard(row_width=4)
        try:
            r = await http.get(
            ...
            try:
                ...
                try:
                    await msg.edit_caption(
                        "😶‍🌫️ Timeout. Task Has Been Cancelled!")
                except MessageIdInvalid:
                    pass
        except Exception as err:
            await query.message.edit_caption(
    ...
    try:
        ...
        if genre := r_json.get("genre"):
            genre = "".join(
                ...
                if i in
                GENRES_EMOJI else f"#{i.replace('-', '_').replace(' ', '_')}, "
        ...
        if thumb := r_json.get("image"):
            ...
            await query.message.edit_caption(res_str,
                                             parse_mode=enums.ParseMode.HTML,
                                             reply_markup=markup)
...
async def imdb_en_callback(self: Client, query: CallbackQuery):
    ...
    try:
        ...
        if r_json.get("actor"):
            actors = actors = "".join(f"<a href='{i['url']}'>{i['name']}</a>, "
                                      for i in r_json["actor"])
