...
async def ceksub(self: Client, ctx: Message, strings):
    if len(ctx.command) == 1:
        return await ctx.reply_msg(
            strings("sub_extr_help").format(cmd=ctx.command[0]),
            quote=True,
            del_in=5)
    link = ctx.command[1]
    start_time = time()
    ...
    try:
        ...
        buttons.append([
            InlineKeyboardButton(strings("cancel_btn"),
...
@app.on_message(
    filters.command(["converttosrt", "converttoass"], COMMAND_HANDLER))
@capture_err
@ratelimiter
@use_chat_lang()
async def convertsrt(self: Client, ctx: Message, strings):
    reply = ctx.reply_to_message
    if not reply or not reply.document or not reply.document.file_name or not reply.document.file_name.endswith(
        (".vtt", ".ass", ".srt")):
        return await ctx.reply_msg(
    ...
    await ctx.reply_document(
        ...
        caption=strings("capt_conv_sub").format(nf=filename,
...
async def stream_extract(self: Client, update: CallbackQuery, strings):
    ...
    try:
        ...
        try:
            ...
            pass
        await update.message.edit_msg(
            strings("fail_extr_sub").format(link=link, e=e))
