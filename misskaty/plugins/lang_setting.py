...
async def chlang(c: Client, m: Union[CallbackQuery, Message], strings):
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        *gen_langs_kb(),
        [
            InlineKeyboardButton(strings("back_btn", context="general"),
    ...
    res = (strings("language_changer_private") if msg.chat.type
           == ChatType.PRIVATE else strings("language_changer_chat"))
...
async def set_chat_lang(c: Client, m: CallbackQuery, strings):
    ...
    strings = partial(
        get_locale_string,
        langdict[lang].get("lang_setting",
                           langdict[default_language]["lang_setting"]),
        lang,
        "lang_setting",
    )
    ...
    if m.message.chat.type == ChatType.PRIVATE:
        ...
        keyboard = None
    await m.message.edit_msg(strings("language_changed_successfully"),
