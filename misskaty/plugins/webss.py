...
async def take_ss(self: Client, ctx: Message, strings):
    ...
    url = ctx.command[1] if ctx.command[1].startswith(
        "http") else f"https://{ctx.command[1]}"
    download_file_path = os.path.join(
        "downloads/", f"webSS_{ctx.from_user.id}.png")
    ...
    try:
        ...
        await gather(
            *[
                ...
                ctx.reply_photo(download_file_path,
                                caption=strings("str_credit")),
            ]
        )
