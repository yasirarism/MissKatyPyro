...
@app.on_message(filters.incoming & ~filters.private
                & filters.command(["inkick"], COMMAND_HANDLER))
@ratelimiter
        ...
        if len(message.command) > 1:
            ...
            async for member in app.get_chat_members(message.chat.id):
                if member.user.is_bot:
                    continue
                if member.user.status.value in input_str and member.status.value not in (
                        "administrator", "owner"):
                    try:
...
@app.on_message(filters.incoming & ~filters.private
                & filters.command(["uname"], COMMAND_HANDLER))
@ratelimiter
...
async def rm_delacc(client, message):
    ...
    if user.status.value in ("administrator", "owner"):
        ...
        await sent_message.edit_msg(
            f"✔️ **Berhasil menendang {count} akun terhapus.**")
    else:
        sent_message = await message.reply_text(
...
@app.on_message(filters.incoming & ~filters.private
                & filters.command(["instatus"], COMMAND_HANDLER))
@ratelimiter
