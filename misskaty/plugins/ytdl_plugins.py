...
@app.on_message(
    filters.command(["ytsearch"], COMMAND_HANDLER) & ~filters.channel)
@capture_err
@ratelimiter
@use_chat_lang()
async def ytsearch(self: Client, ctx: Message, strings):
    ...
    btn = InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(strings("dl_btn"),
                                 callback_data=f"yt_gen|{i['id']}")
        ],
    ])
    img = await get_ytthumb(i["id"])
    caption = out
    markup = btn
    await ctx.reply_photo(img,
                          caption=caption,
                          reply_markup=markup,
                          quote=True)
...
async def ytdownv2(self: Client, ctx: Message, strings):
    ...
    async with iYTDL(log_group_id=0,
                     cache_path="cache",
                     ffmpeg_location="/usr/bin/ffmpeg") as ytdl:
        try:
            ...
            markup = x.buttons
            photo = x.image_url
            msg = await ctx.reply_photo(photo,
                                        caption=caption,
                                        reply_markup=markup,
                                        quote=True)
...
async def ytdl_listall_callback(self: Client, cq: CallbackQuery, strings):
    ...
    async with iYTDL(log_group_id=0,
        ...
        await cq.edit_message_media(media=media,
                                    reply_markup=buttons.add(cq.from_user.id))
...
async def ytdl_gendl_callback(self: Client, cq: CallbackQuery, strings):
    ...
    async with iYTDL(
        ...
        try:
            ...
            uid, disp_str = ytdl.get_choice_by_id(match[2],
                                                  media_type,
                                                  yt_url=yt_url)
...
async def ytdl_scroll_callback(self: Client, cq: CallbackQuery, strings):
    ...
    await cq.edit_message_media(InputMediaPhoto(await get_ytthumb(i["id"]),
                                                caption=out),
                                reply_markup=btn)
