...
def puasa():
    ...
    x = datetime(int(tahun), int(bulan), int(tgl),
...
async def mute_chat(chat_id: int):
    ...
    try:
        ...
        await app.send_message(
            LOG_CHANNEL,
            langdict[getlang]["nightmodev2"]["nmd_on_err"].format(
                chat_id=chat_id, e=e),
        )
    else:
...
async def nightmode_handler(c, msg, strings):
    ...
    try:
        start_timestamp = TIME_ZONE.localize(datetime.strptime(
            (now.strftime("%m:%d:%Y - ") + start), "%m:%d:%Y - %H:%M"))
    except ValueError:
...
async def callbackanightmd(c, q, strings):
    await q.answer(
        strings("nmd_cb").format(bname=c.me.first_name,
                                 ver=__version__, pyver=platform.python_version()),
        show_alert=True,
    )
