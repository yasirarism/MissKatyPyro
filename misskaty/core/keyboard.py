...
from bot.helper.functions import get_urls_from_text as is_url
...
def keyboard(buttons_list, row_width: int = 2):
    ...
    data = [(Ikb(text=str(i[0]), callback_data=str(i[1])) if not is_url(i[1]) else Ikb(text=str(i[0]), url=str(i[1]))) for i in buttons_list]
    buttons.add(*data)
    return buttons
