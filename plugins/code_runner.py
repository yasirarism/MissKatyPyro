...
async def list_lang(client, message):
  req = requests.get("https://glot.io/api/run")
  parse = req.json()
  list_ = "".join(f"~> {i['name']}\n" for i in parse)
  return await message.reply(f"Daftar Bahasa Pemrograman Yang Didukung:\n{list_}")

@Client.on_message(filters.command(["assembly","assembly@MissKatyRoBot"], "!"))
async def assembly(client, message):
    if len(message.command) < 2 :
        return await message.reply("Silahkan masukkan kode yang ingin dijalankan.")
    try:
      siteurl = "https://glot.io/api/run/assembly/latest"
...
@Client.on_message(filters.command(["coffeescript","coffeescript@MissKatyRoBot"], "!"))
async def coffeescript(client, message):
    ...
    try:
      ...
      data = {"files":
              [
                {"name": "Yasir.coffee", "content": code}
              ]
             }
      response = requests.post(siteurl, json=data, headers=headers)
      response = response.json()
...
async def groovy(client, message):
    ...
    try:
      ...
      data = {"files":
              [
                {"name": "Yasir.groovy", "content": code}
              ]
             }
      response = requests.post(siteurl, json=data, headers=headers)
      response = response.json()
...
@Client.on_message(filters.command(["javascript","javascript@MissKatyRoBot"], "!"))
...
@Client.on_message(filters.command(["typescript","typescript@MissKatyRoBot"], "!"))
